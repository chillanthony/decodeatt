...
# s64 variant: 64 samples per problem by default. These are only defaults and
# can still be overridden by exporting the same variables.
SAMPLES="${NUM_RETURN_SEQUENCES:-64}"
...
for index in "${!SCRIPTS[@]}"; do
  ...
  (
    ...
    export BATCH_SIZE PROBLEM_BATCH_SIZE
