...
echo "[$RUN_NAME] checking cached dataset before loading the model"
if ! "$PYTHON_BIN" - "$DATASET" <<'PY'
...
dataset = sys.argv[1]
problems = load_problems(dataset, 1)
print(f"[dataset-check] dataset={dataset} cached_problems={len(problems)}")
PY
then
  echo "Dataset cache check failed." >&2
  echo "Download AIME first with: scripts/jiqun/download_aime24.sh" >&2
  exit 1
fi
