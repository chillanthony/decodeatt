...
2. 统一 `kvbench.diagnostics.gen_traces` 与当前 KVBench 的模型、数据和 prompt 配置。
...
- `kvbench.diagnostics.gen_traces` 已统一复用 KVBench 的模型加载、AIME 数据加载和 prompt 构造逻辑。
...
- 新增 `scripts/tideprobe/gen_traces_8gpu.sh`，通过 `torchrun` 将 10 道题静态分配到 8 张 A100；已有完整 trace 默认跳过，文件采用原子写入，支持任务断点续跑。
- 全量测试通过，共 39 项；Python 编译、CLI 和 shell 语法检查通过。

运行命令：

```bash
bash scripts/tideprobe/gen_traces_8gpu.sh
```
...
- 新增 `scripts/tideprobe/extract_signals_8gpu.sh`，使用 8 张 A100 按 trace 静态分片提取信号，已有完整信号文件默认跳过，支持断点续跑。
- 新增 `kvbench/diagnostics/transition_detection.py`：对 logit entropy 变化、attention entropy 变化和 KV delta 做滚动标准化、局部峰值检测与等权融合。
...
bash scripts/tideprobe/extract_signals_8gpu.sh

# 3. 在 CPU 上检测、评估并画图
uv run python -m kvbench.diagnostics.transition_detection \
...
- 新增 `scripts/tideprobe/layer_sensitivity_8gpu.sh`，按 trace/probe 在 8 张 A100 上静态分片；不同主实验、细扫和辅助实验使用独立 run tag，不会互相覆盖 shard。
- 新增 `kvbench/diagnostics/layer_analysis.py`，输出 `layer_sensitivity.csv`、聚合 CSV、layer $\times$ budget 热力图、敏感曲线和 `layer_sensitivity_metrics.json`；指标包含跨 trace 和跨 random seed 的层排序 Spearman 稳定性。
...
- 新增 `kvbench/diagnostics/eviction_scoring.py`，在固定轨迹上 teacher forcing 扫描 R-KV、SnapKV、Window、Random 的预算 $512/1024/1536$；FullKV 逐 token NLL 直接复用公共轨迹作为共享对照。
- 新增 `scripts/tideprobe/eviction_alignment_8gpu.sh`：8 个 rank 按四种策略分为四个子组，每种策略由两个 rank 分摊 trace $\times$ budget 任务；每个原始 `.pt` 结果独立原子写入并默认跳过已有文件，可断点续跑。
- 新增 `kvbench/diagnostics/eviction_alignment.py`：按过渡点 $\pm16$ token 窗口计算每次驱逐的 transition-hit 比例，再计算驱逐后 32 token 相对 FullKV 的 NLL 增量。
...
```bash
uv run python -m kvbench.diagnostics.eviction_alignment \
  --config configs/experiments/tideprobe_step1.yaml
```
...
- 新增 `kvbench/diagnostics/sparse_cliff.py`，直接复用实验 3 的逐 token sparse NLL 与公共 FullKV NLL，不增加新的 GPU forward。
...
- 新增 `scripts/tideprobe/sparse_cliff.sh` 作为 CPU 分析入口；全量测试共 50 项通过，合成三预算曲线已验证 CF、阈值预算和单调性诊断。
...
```bash
bash scripts/tideprobe/experiments_2_4.sh
```
...
1. 统一 `kvbench.diagnostics.gen_traces` 与当前 `kvbench` 的模型、数据和生成配置。
2. 增加过渡信号提取与事件检测脚本。
